...
function distToSymbol(p: number, line: string) {
  for (let i = p + 1; i < line.length; i++) {
    if (!isNumber(line[i])) return Math.abs(i - p);
  }
  return false;
}

function seekNumber(direction: Dir, point: number, line: string) {
...
function explode(point: number, line: string) {
  ...
  line = `${line.substring(0, seekR[0])}${right ? right : ""}${
    seekR[0] < line.length ? line.substring(seekR[0] + 1) : ""
  }`;

  line = `${line.substring(0, point)}0${line.substring(point + 5)}`;

  line = `${seekL[0] >= 0 ? line.substring(0, seekL[0]) : ""}${
    left ? left : ""
  }${line.substring(seekL[0] + 1)}`;
  return line;
}

function reduce(line: string) {
  while (true) {
    let gamed = false;
    let depth = 0;
    //@ts-ignore
    for (const index in line) {
      ...
      if (number > 9) {
        line = `${line.substring(0, i)}[${split(number)}]${line.substring(
...
function add(line1: string, line2: string) {
  return `[${line1},${line2}]`;
}
